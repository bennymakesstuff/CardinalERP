...
$arrayfields=array(
	...
	'c.date_commande'=>array('label'=>$langs->trans("Veh.Order Date"), 'checked'=>1),
...
*/
	...
	'c.cfd_ref'=>array('label'=>$langs->trans("Purchase Order"), 'checked'=>1),
...
if (! empty($arrayfields['c.cfd_ref']['checked']))			print_liste_field_titre($arrayfields['c.cfd_ref']['label'],$_SERVER["PHP_SELF"],'c.cfd_ref','',$param,'',$sortfield,$sortorder);
...
*/
	...
	if ($orderstatic->fk_project)
	{
		$projectstatic->fetch($orderstatic->fk_project); 
//		$projectstatic->societe->fetch($projectstatic->societe->id);
	}
	if ($projectstatic->socid)
	{
		$socstatic->fetch($projectstatic->socid);
	}
...
/*
	...
	if (! empty($arrayfields['c.datec']['checked']))
	{
		...
		print dol_print_date($db->jdate($objc->date_creation), 'dayhour', 'tzuser');
		print '</td>';
	...
	{
		print '<td align="center" class="nowrap">';
		print dol_print_date($db->jdate($objc->date_update), 'dayhour', 'tzuser');
		print '</td>';
	...
	{
		print '<td align="center">'.yn($objc->billed).'</td>';
		if (! $i) $totalarray['nbfield']++;
	}
*/
/*
...
*/
	...
	// Purchase Order Ref
	if (! empty($arrayfields['c.cfd_ref']['checked']))
	{
