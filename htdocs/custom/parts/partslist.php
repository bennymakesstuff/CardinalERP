 ...
 * Copyright (C) 2018-2019	Peter Roberts		<webmaster@finchmc.com.au>
 *
...
/**
 *      \file       htdocs/custom/parts/parts.php
 *      \ingroup    projet
...
/*
$partslists=Array();
$partslists = _getProjectsWithParts();
print_r($partslists);
if (count($partslists) > 0) print '<br/><br/>No of Partslists = '.count($partslists).'<br/><br/>';
...
$langs->loadLangs(array('users', 'projects', 'companies', 'suppliers', 'compta', 'orders', 'parts@parts','other'));
...
$projmode	= ($id == '' && $projectid == '' && $ref == '')?'listall':'listone';		// Mode is 'listone' if there is an individual project of interest, else 'listall'
...
// Can't use generic include because when creating a project, ref is defined and we dont want error if fetch fails from ref.
if ($id > 0 || ! empty($ref))
{
...
$hookmanager->initHooks(array('partslist'));
...
$parameters=array('id'=>$id, 'socid'=>$socid, 'projectid'=>$projectid);
...
*/
	...
	// Buttons
	if (GETPOST('button_removefilter_x','alpha') || GETPOST('button_removefilter.x','alpha') || GETPOST('button_removefilter','alpha') || GETPOST('button_search_x','alpha') || GETPOST('button_search.x','alpha') || GETPOST('button_search','alpha'))  // All tests are required to be compatible with all browsers
	{
		$massaction='';
		$toselect='';
...
// PJR TODO		if (GETPOST('model')) $object->setDocModel($user, GETPOST('model','alpha'));

//$id = 276;						// for testing
//$object = new Project($db);		// for testing
//$object->fetch($id);			// for testing
//$freddo = $id;			// for testing
...
//			$action = '';
//			$action = 'broken';	// PJR TODO - reinstate above line after testing
		}
	}
/*
...
if ($projmode == 'listall')  // all projects
{
...
if ($projmode != 'listall')  // project card only printed for individual projects
{
...
// New Purchase Requisition Button
$newcardbutton='';
...
if ($projmode == 'listall') print '<input type="hidden" name="projmode" value="listall">';
...
if (! empty($moreforfilter) && $projmode == 'listall')
{
...
$partslists=Array();
$partslists = _getProjectsWithParts($projectid>0?$projectid:0);

//print '<br>$projectid = '.$projectid.'<br>';
//print_r($partslists);
//if (count($partslists) > 0) print '<br/><br/>No of Partslists = '.count($partslists).'<br/><br/>';
...
foreach($partslists as $partslist)
{
	...
	// Project Title Line
//	if ($projmode == 'listall' && $previousproj != $currentproj)
//	{
		print '<tr class="trforpartsbreak">'."\n";
		print '<td class="nowrap" '.$titlecolspan.'>';  // PJR TODO

		if ($projectstatic->id)  // PJR TO DO - not needed with new coding regime
		{
		...
		else // PJR TO DO - not needed with new coding regime
		{
			$previousproj = 0;
...
// lines
// ====================================================
	$parts_lines = array(array());
 	$parts_lines = _getPartsArray($id, 0, $sortfield, $sortorder, $morewherefilter, $only_product, $page, $limit, $offset);
//echo '<br/>COUNT: '.count($parts_lines);

	$previous_prid = -1;
	$previous_poid = -1;
		
	foreach($parts_lines as $parts_line)
	{
		$requisitionstatic = new Commande($db);
		$productstatic = new Product($db);	
		
		if ($parts_line->fk_commande) $requisitionstatic->fetch($parts_line->fk_commande);
		if ($parts_line->fk_product) $productstatic->fetch($parts_line->fk_product);
		...
		if ($line->cfd_fk_commande)
		{
			...
			$purchaseorderstatic->fetch($parts_line->cfd_fk_commande);
		...
		$current_prid = (! empty($parts_line->fk_commande)?$parts_line->fk_commande:0);
		if ($previous_prid != $current_prid)
		{
			_print_PurchReq_heading($current_prid, $colspan); // print Purchase Requisition Heading if different from previous
			$previous_poid = -1;	// Reset Purchase Order check beneath new PR heading
		}
		$previous_prid = $current_prid;

		//  ==== PURCHASE ORDER LEVEL ====
		$current_poid = (! empty($parts_line->cfd_fk_commande)?$parts_line->cfd_fk_commande:0);
		if ($previous_poid != $current_poid) _print_PurchOrd_heading($current_poid, $colspan); // print Purchase Order Heading if different from previous
		$previous_poid = $current_poid;

// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX

		//  ==== PURCHASE REQ LINE ITEM LEVEL ====
//		_print_PurchReq_lineitem($prlineid);
		print '<tr class="oddeven" id="row-'.$parts_line->rowid.'">';
			
		// Line in view mode
		if(! empty($arrayfields['c.line']['checked']))
		{
			print '<td></td><td class="linecolnum nowrap" align="left">';
			if ($type==1) $text = img_object($langs->trans('Service'),'service');
			else $text = img_object($langs->trans('Product'),'product');
			$text.= ' <strong><small>PR: </small></strong>';

			print $text.' '.$parts_line->rowid.'</td>';
		}
			
		// Show product and description
		$type=(! empty($parts_line->product_type)?$parts_line->product_type:$parts_line->fk_product_type);
		// Part description
		if(! empty($arrayfields['c.desc']['checked']))
		{
			print '<td class="linecoldescription minwidth150imp" colspan = "2">';
			if (! empty($parts_line->cfd_desc))
			{
				print dol_htmlentitiesbr($parts_line->cfd_desc);
			}
			elseif (! empty($parts_line->description))
			{
				print dol_htmlentitiesbr($parts_line->description);
			}
			else
			{
		...
		if(! empty($arrayfields['c.qty']['checked']))
		{
			// following based on $parts_line->getLabelOfUnit('')
			$label_type = 'short_label';
			$sqlu = 'SELECT '.$label_type.' FROM '.MAIN_DB_PREFIX.'c_units WHERE rowid='.$parts_line->fk_unit;
			...
			print '<td align="right" class="linecolnum nowrap">'.$parts_line->qty.$label.'</td>';
		}
...
/*					print '<table class="nobordernopadding"><tr class="nocellnopadd">';
	...
	//					$viewstatut = 2;
						...
						if (!empty($purchaseorderstatic->note_private) || !empty($purchaseorderstatic->note_public))
						{
							print ' <span class="note">';
		...
		if (! empty($arrayfields['cf.req_vend']['checked']))
		{
			...
			print '<table class="nobordernopadding"><tr class="nocellnopadd">';
				// Requester
				print '<td>';
				if ($parts_line->cef_requester)
				{
					$userstatic = new User($db);
					$userstatic->fetch($parts_line->cef_requester);
					print $userstatic->getNomUrl(1);
				}
				if ($parts_line->cef_ticket)
				{
					$ticketstatic = new Ticket($db);
					$ticketstatic->fetch($parts_line->cef_ticket);
		...
		if (! empty($arrayfields['cf.vendor_refs']['checked']))
		{
			...
			print '<table class="nobordernopadding"><tr class="nocellnopadd">';
				...
				// Vendor Part No.
				print '<td>'.$parts_line->cfd_partno_ref.'</td>'."\n";
			print '</tr></table>';
	...
	*/
	...
	/*
		...
		if (! empty($arrayfields['cf.total_ht']['checked']))
		{
			...
			if ($purchaseorderfound)
			{
				print price($parts_line->cfd_total_ht);
			}
			else
			{
				print '';
		...
		if (! empty($arrayfields['cf.total_vat']['checked']))
		{
			...
			if ($purchaseorderfound)
			{
				print price($parts_line->cfd_total_tva);
			}
			else
			{
				print '';
		...
		if (! empty($arrayfields['cf.total_ttc']['checked']))
		{
			...
			if ($purchaseorderfound)
			{
				print price($parts_line->cfd_total_ttc);
			}
			else
			{
				print '';
...
$urlsource=$_SERVER["PHP_SELF"]./*'?projmode=listall'.*/'#builddoc';
$genallowed=1;
$delallowed=1;
$modelpdf=partslist;
...
/**
 * 	Return list of projects with known parts needed
 *
 *  @param	int		$projectid	Project id (0 = all projects)
 * 	@return	array				List of projects
 */
function _getProjectsWithParts($projectid=0)
{
	global $db, $conf;

	//	Projects with known parts orders
	$partslists = array(array());
	$sql = 'SELECT ';
	$sql.= ' p.rowid AS projectid, p.fk_soc AS customer, p.ref AS ref';
	...
	if ($projectid>0) $sql.= ' AND c.fk_projet = '.$projectid;
	else $sql.= ' AND c.fk_projet IS NOT NULL';
	...
	if ($projectid>0) $sql.= ' AND cf.fk_projet = '.$projectid;
	else $sql.= ' AND cf.fk_projet IS NOT NULL';
	...
	if ($resql)
	{
		...
		if ($nump)
		{
			...
			while ($i < $nump)
			{
				...
				$partslists[$i]['projectid']	=	$obj->projectid;
				$partslists[$i]['customer']		=	$obj->customer;
				$partslists[$i]['ref']			=	$obj->ref;
 ...
 * @return	int						0
 */
function _print_PurchReq_heading($prid, $colspan)
{
	global $db, $conf, $langs;

	$requisitionstatic = new Commande($db);
	$requisitionstatic->fetch($prid);
	$formfile = new FormFile($db);

	print '<tr class="oddeven" id="pr-row-'.$prid.'">';

	// Purchase Requisition
	$purchreqcolspan = 'colspan = "'.($colspan-1).'"';
	print '<td class = "left"><strong>PR:</strong></td>';
	print '<td class="nowrap" '.$purchreqcolspan.'>';  // PJR TODO
	print '<table class="nobordernopadding"><tr class="nocellnopadd">';
		// Purchase Requisition Ref
		print '<td class="nowrap">';
		if ($prid > 0)
 ...
 * @return	int						0
 */
function _print_PurchOrd_heading($poid, $colspan)
{
	global $db, $conf, $langs;

	$purchaseorderstatic = new CommandeFournisseur($db);
	$purchaseorderstatic->fetch($poid);
	$formfile = new FormFile($db);

	// Purchase Order Line
	print '<tr class="oddeven" id="pr-row-'.$poid.'">';

	// Purchase Requisition
	$purchordcolspan = 'colspan = "'.($colspan-1).'"';
	print '<td class = "nowrap right">'.img_picto('', 'rightarrow').'PO:</td>';
	print '<td class="nowrap" '.$purchordcolspan.'>';  // PJR TODO
	print '<table class="nobordernopadding"><tr class="nocellnopadd">';
		// Purchase Order Ref
		print '<td class="nowrap">';
		if ($poid > 0)
		...
		print '<table class="nobordernopadding"><tr class="nocellnopadd">';
			print '<td align="left" class="nowrap">'.$purchaseorderstatic->LibStatut($purchaseorderstatic->statut, 3, $purchaseorderstatic->billed).'</td>';
			print '<td class="nobordernopadding nowrap">';
			print $purchaseorderstatic->getNomUrl(1, 0, 0, 0, 0, 1);
			print '</td>';
			...
			{
				print ' <span class="note">';
				print '<a href="'.DOL_URL_ROOT.'/commande/note.php?id='.$purchaseorderstatic->id.'">'.img_picto($langs->trans("ViewPrivateNote"),'object_generic').'</a>';
				print '</span>';
			}
			print '</td>';
		...
		{
			print img_warning("Not yet ordered").' Not yet ordered';
		}
		print '</td>';
	print '</tr></table>';
	print '</td>';
 ...
 * @return	int						0
 */
function _print_PurchReq_lineitem($prlineid)
{
	global $db, $conf, $langs;
...
/**
 *	Return list of Purchase Requisition lines and for related Purchase Orders
 *
 *	@param		int		$prid			Purchase Requisition ID (rowid)
 *	@param		int		$only_product	Return only physical products
 * 	@return		array					List of PR lines
 */
function _fetch_prlines($prid=0, $poid=0, $only_product=0)
{
	global $db, $conf, $langs;
...
*/
	...
	$sql = 'SELECT ';
	$sql.= '  cd.rowid, cd.fk_product, cd.fk_parent_line, cd.product_type, cd.fk_commande, cd.label as custom_label, cd.description, cd.price, cd.qty, cd.vat_src_code, cd.tva_tx';
	$sql.= ', cd.localtax1_tx, cd.localtax2_tx, cd.localtax1_type, cd.localtax2_type, cd.fk_remise_except, cd.remise_percent, cd.subprice, cd.fk_product_fournisseur_price as fk_fournprice, cd.buy_price_ht as pa_ht, cd.rang, cd.info_bits, cd.special_code';
	$sql.= ', cd.total_ht, cd.total_ttc, cd.total_tva, cd.total_localtax1, cd.total_localtax2, cd.date_start, cd.date_end';
	$sql.= ', cd.fk_unit,';
	$sql.= ', cd.fk_multicurrency, cd.multicurrency_code, cd.multicurrency_subprice, cd.multicurrency_total_ht, cd.multicurrency_total_tva, cd.multicurrency_total_ttc';
	$sql.= ', prd.ref as product_ref, prd.description as product_desc, prd.fk_product_type, prd.label as product_label, prd.tobatch as product_tobatch';
	$sql.= ', prd.weight, prd.weight_units, prd.volume, prd.volume_units';

	$sql.= ', cfd.rowid AS cfd_rowid';
	$sql.= ', cfd.fk_commande AS cfd_fk_commande';
	//$sql.= ', cfd.fk_parent_line AS cfd_fk_parent_line';
	$sql.= ', cfd.ref AS cfd_partno_ref';
	$sql.= ', cfd.label AS cfd_label';
	$sql.= ', cfd.description AS cfd_description';
	$sql.= ', cfd.qty AS cfd_qty';
	$sql.= ', cfd.fk_product AS cfd_fk_product';
	
	$sql.= ', cfd.product_type AS cfd_product_type';
	$sql.= ', cfd.fk_unit AS cfd_fk_unit';
	$sql.= ', cfd.total_ht AS cfd_total_ht';
	$sql.= ', cfd.total_tva AS cfd_total_tva';
	$sql.= ', cfd.total_ttc AS cfd_total_ttc';	
	...
	$sql.= ' WHERE 1 = 1';
	$sql.= ' AND cd.fk_commande = '.$prid;
	if ($only_product) $sql .= ' AND prd.fk_product_type = 0';
	$sql .= ' ORDER BY cd.rang, cd.rowid';

	dol_syslog(get_class()."::fetch_lines", LOG_DEBUG);
	$result = $db->query($sql);
	if ($result)
	{
		...
		while ($i < $num)
		{
			...
			$line->fetch_optionals();  // PJR from v8.0.3
...
*/
	 ...
	 */
    function _get_element_list($type, $tablename, $datefieldname='', $dates='', $datee='')
    {
        $elements = array();
...
/**
 * Return list of parts for one particular project
 * Default sort order is on Purchase Requisition, then on Purchase Order, then on Purchase Requisition line. 
 * Occasionally, multiple POs might be raised for one PR line and this needs to be handled by exception with an alert.
 *
...
function _getPartsArray($projectid=0, $orphans=0, $sortfield, $sortorder, $morewherefilter='', $only_product=0, $page, $limit, $offset)
//function _getPartsArray($usert=null, $userp=null, $projectid=0, $socid=0, $mode=0, $filteronproj='', $filteronprojstatus='-1', $morewherefilter='',$filteronprojuser=0,$filterontaskuser=0)
{
	global $db, $conf, $langs;
	...
	//		$projectid = 276;
	if ($projectid > 0)	// if projectid is provided we want specific project
	{
		$sql1.= ' AND c.fk_projet = '.$projectid;
		$sql2.= ' AND cf.fk_projet = '.$projectid;
	}
	else // we are either wanting parts for a specific project or orphan parts
	{
		if ($orphans >0) // if greater than zero we are looking for orphans
		{
			$sql1.= ' AND c.fk_projet IS NULL';
			$sql2.= ' AND cf.fk_projet IS NULL';
		}
	}
	...
	if ($projmode == 'listall' /*|| $action == 'builddoc'*/)
	{
		if (! $sortfield) $sortfield='cd_project_ref, c_rowid, cd_rowid, cf_rowid, cfd_rowid';
