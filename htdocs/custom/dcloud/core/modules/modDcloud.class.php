...
*/
		...
		/* BEGIN LEFTMENU DCLOUD IN ECM */
		$this->menu[$r++]=array(
			'fk_menu'=>'fk_mainmenu=ecm',	// '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			'type'=>'left',							// This is a Left menu entry
			'titre'=>'Dropbox',
			'mainmenu'=>'ecm',
			'leftmenu'=>'dcloud',
			'url'=>'/dcloud/document.php',
			'langs'=>'dcloud@dcloud',	        	// Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>104,
			'enabled'=>'$conf->dcloud->enabled && !empty($conf->global->DROPBOX_ACCESS_TOKEN)',  	// Define condition to show or hide menu entry. Use '$conf->mymodule->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms'=>'$user->rights->dcloud->read',
			'target'=>'',
			'user'=>2);								// 0=Menu for internal users, 1=external users, 2=both

		/* END LEFTMENU DCLOUD IN ECM */
  	}
