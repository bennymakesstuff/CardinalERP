 ...
 *		\ingroup    timecard othermodule1 othermodule2
...
include 'core/lib/includeMain.lib.php';
 if(!$user->rights->timecard->approval->admin){
...
if (!$removefilter )		// Both test must be present to be compatible with all browsers
{
	...
	if($ls_user_approval==-1)$ls_user_approval='';
        $ls_timsheetuser= GETPOST('ls_timecarduser','int');
...
//$upload_dir = $conf->timecard->dir_output.'/Timecarduser/'.dol_sanitizeFileName($object->ref);


 // uncomment to avoid resubmision
//if(isset( $_SESSION['Timecarduser_class'][$tms]))
//{
...
if ($user->societe_id > 0 || 
       (!$user->rights->timecard->add && ($action=='add' || $action='create')) ||
       (!$user->rights->timecard->view && ($action=='list' || $action='view')) ||
       (!$user->rights->timecard->delete && ($action=='confirm_delete')) ||
       (!$user->rights->timecard->edit && ($action=='edit' || $action='update')))
{
	accessforbidden();
}
*/
...
if ($cancel){
    ...
    $tms=time();
    $_SESSION['Timecarduser_'.$tms]=array();
    $_SESSION['Timecarduser_'.$tms]['action']=$action;
            
}else if (($action == 'add') || ($action == 'update' && ($id>0 || !empty($ref))))
{
        //block resubmit
        if(empty($tms) || (!isset($_SESSION['Timecarduser_'.$tms]))){
		...
		$object->timecarduser=GETPOST('Timecarduser');
 ...
 }else if ($id==0 && $ref=='' && $action!='create') 
 {
  ...
  switch($action){		
                    case 'update':
                            ...
                            {
                                // Creation OK
                                unset($_SESSION['Timecarduser_'.$tms]);
                    ...
                    case 'edit':
                            ...
                            if ($id > 0 || !empty($ref) )
                            {
                                    ...
                                    if($result > 0)$result=$object->fetchTaskTimecard();
...
if(isset( $_SESSION['Timecarduser_class'][$tms]))
{
    unset($_SESSION['Timecarduser_class'][$tms]);
}
...
llxHeader('',$langs->trans('TimecardUser'),'','','','',$morejs);
...
switch ($action) {
   ...
   case 'delete';
        if( $action=='delete' && ($id>0 || $ref!="")){
         $ret=$form->form_confirm($PHP_SELF.'?action=confirm_delete&id='.$id,$langs->trans('DeleteTimecarduser'),$langs->trans('ConfirmDelete'),'confirm_delete', '', 0, 1);
    ...
    case 'view':
    {
        ...
        if($edit==0 && $new==0){ //show tabs
            $head=Timecarduser_prepare_head($object);
            dol_fiche_head($head,'card',$langs->trans('Timecarduser'),0,'timecard@timecard');            
        }else{
            print_fiche_titre($langs->trans('Timecarduser'));
        }
...
// show the field status
		...
		if($edit==1){
		print select_enum('timecard_user','status','Status',$object->status);
		}else{
...
// show the field target
		...
		if($edit==1){
		print select_enum('timecard_user','target','Target',$object->target);
		}else{
...
// show the field timsesheetuser
		...
		print '<td>'.$langs->trans('Timecarduser').' </td><td>';
                $sqltail= "JOIN ".MAIN_DB_PREFIX."user AS u ON u.rowid=t.fk_userid";
		if($edit==1){
                    //avoid full table scan print select_generic('timecard_user', 'rowid','Timecarduser','CONCAT(u.lastname," ",u.firstname) AS username','CONCAT(t.year_week_date," ",t.target) AS weektarget',$object->timecarduser,' - ',$sqltail);
                    print '<input class="flat" size="5" type="text" name="Timecarduser" value="'.$object->timecarduser.'"/>';

		}else{
                    print print_generic('timecard_user', 'rowid',$object->timecarduser,'CONCAT(lastname," ",firstname) AS username','CONCAT(year_week_date," ",target) AS weektarget',' - ','',$sqltail);
                    //print $object->project_tasktime_list;
...
//		print "<tr>\n";
	...
	print '<div class="center">';
        ...
        print_fiche_titre($langs->trans('Timecarduser'));
        $head=Timecarduser_prepare_head($object);
        dol_fiche_head($head,'info',$langs->trans("Timecarduser"),0,'timecard@timecard');            
	...
	$object->fetch_thirdparty();
        ...
        $head=Timecarduser_prepare_head($object);
        dol_fiche_head($head,'documents',$langs->trans("Timecarduser"),0,'timecard@timecard');            
    ...
    $sql.= ' FROM '.MAIN_DB_PREFIX.'timecard_user as t';
    $sqlwhere='';
...
{
        $sqlcount='SELECT COUNT(*) as count FROM '.MAIN_DB_PREFIX.'timecard_user as t';
        if(!empty($sqlwhere))
	...
	if (!empty($ls_user_approval))	$param.='&ls_user_approval='.urlencode($ls_user_approval);
        ...
        print_barre_liste($langs->trans("Timecarduser"),$page,$PHP_SELF,$param,$sortfield,$sortorder,'',$num,$nbtotalofrecords);
...
//Search field forstatus
	print '<td class="liste_titre" colspan="1" >';
		print select_enum('timecard_user','status','ls_status',$ls_status);
	print '</td>';
//Search field fortarget
//	print '<td class="liste_titre" colspan="1" >';
//		print select_enum('timecard_user','target','ls_target',$ls_target);
//	print '</td>';
...
//Search field foruser_approval
    ...
    print ' '.$langs->trans('Timecarduser')."</a>\n";
...
function Timecarduser_prepare_head($object)
{
    ...
    // $this->tabs = array('entity:+tabname:Title:@timecard:/timecard/mypage.php?id=__ID__');   to add new tab
    // $this->tabs = array('entity:-tabname);   												to remove a tab
    complete_head_from_modules($conf,$langs,$object,$head,$h,'timecard');
    complete_head_from_modules($conf,$langs,$object,$head,$h,'timecard','remove');
