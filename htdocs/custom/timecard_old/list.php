...
$search_progress=GETPOST('search_progress');
...
$arrayfields=array(
	...
	//'t.fms_status'=>array('label'=>$langs->trans("Status"), 'checked'=>1, 'position'=>1000),
);
// Extra fields
...
if (empty($search_taskstatus) && $search_taskstatus == '') $search_taskstatus=1;
if (empty($search_progress)) $search_progress=4;
...
switch ($search_progress) {
   case 1:
		 $sql .= " AND t.progress = 0";
		 break;
   case 2:
		 $sql .= " AND (t.progress BETWEEN 1 AND 99)";
		 break;
   case 3:
...
if ($search_progress != '') $param.='&search_progress='.$search_progress;
...
if (! empty($arrayfields['p.ref']['checked']))
{
	...
	print '<input type="text" class="flat" name="search_project_ref" value="'.$search_project_ref.'" size="10">&nbsp;Proj.ref.';
	print '<br>';
/*	print '</td>';
}
...
{
	print '<td class="liste_titre">';*/
	print '<input type="text" class="flat" name="search_project_title" value="'.$search_project_title.'" size="10">&nbsp;Proj.Title';
	print '</td>';
}
if (! empty($arrayfields['s.nom']['checked']))
{
	print '<td class="liste_titre">';
	print '<input type="text" class="flat" name="search_societe" value="'.dol_escape_htmltag($search_societe).'" size="12">';
	print '</td>';
}
...
{
	print '<td class="liste_titre">';
	print '<input type="text" class="flat" name="search_task_ref" value="'.dol_escape_htmltag($search_task_ref).'" size="8">';
	print '</td>';
}
...
{
	print '<td class="liste_titre center">';
	$arrayofprogress = array();
	//foreach($object->progress as $key => $val) $arrayofprogress[$key]=$langs->trans($val);
	$arrayofprogress['1']=$langs->trans("Not Started");
	$arrayofprogress['2']=$langs->trans("Underway");
	$arrayofprogress['3']=$langs->trans("Completed");
	$arrayofprogress['4']=$langs->trans("Incomplete");
	$arrayofprogress['5']=$langs->trans("Null");
	print $form->selectarray('search_progress', $arrayofprogress, $search_progress, 1, 0, 0, '', 0, 0, 0, '', 'maxwidth100');
	print '</td>';
}
...
/*			print '</td>';
		...
		if (! empty($arrayfields['p.title']['checked']))
		{
			print '<td>';*/
			print '<br>';
			print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<small>'.dol_trunc($obj->projecttitle,80).'</small>';
			print '</td>';
			if (! $i) $totalarray['nbfield']++;
		}
