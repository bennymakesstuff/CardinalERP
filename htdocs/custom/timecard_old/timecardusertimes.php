...
*/
			...
			$picto='calendarweek';
			$head = array();
			$head[0][0] = DOL_URL_ROOT."/custom/timecard/timecardusertimes.php";
			$head[0][1] = $langs->trans("All Fortnightly Timesheets");
			$head[0][2] = 'alltimesheets';
			$head[1][0] = DOL_URL_ROOT."/custom/timecard/timecardsingleusertimes.php";
...
//		'<a href="'.DOL_URL_ROOT.'/projet/tasks/time.php?id='.$lines[$i]->id.($showproject?'':'&withproject=1').'">'
					...
					print '<td width="6%" align="center" class="hide'.$i.'"><a href="'.DOL_URL_ROOT.'/custom/timecard/timecard.php?userid='.$userid.'&amp;year='.$kdate_year.'&amp;month='.$kdate_month.'&amp;day='.$kdate_day.'">'.$kdate_date_a.'<br>'.$kdate_date_drf.'</a></td>';
				}
