...
require_once 'class/timecardwhitelist.class.php';
...
$langs->load("Timecardwhitelist_class");
...
//$upload_dir = $conf->timecard->dir_output.'/Timecardwhitelist/'.dol_sanitizeFileName($object->ref);


 // uncomment to avoid resubmision
//if(isset( $_SESSION['Timecardwhitelist_class'][$tms]))
//{
...
$object=new Timecardwhitelist($db);
if($id>0)
{
    $object->id=$id; 
}
...
if ($cancel){
    ...
    {
        $tms=time();
        $_SESSION['Timecardwhitelist_'.$tms]=array();
        $_SESSION['Timecardwhitelist_'.$tms]['action']=$action;
    }else {
...
{
        //block resubmit
        if(empty($tms) || (!isset($_SESSION['Timecardwhitelist_'.$tms]))){
 ...
 }else if ($id==0 && $ref=='' && $action!='create') 
 {
  ...
  switch($action){		
                    case 'update':
                            ...
                            {
                                // Creation OK
                                unset($_SESSION['Timecardwhitelist_'.$tms]);
...
if(isset( $_SESSION['Timecardwhitelist_class'][$tms]))
{
    unset($_SESSION['Timecardwhitelist_class'][$tms]);
}
...
switch ($action) {
   ...
   case 'delete';
        if( $action=='delete' && ($id>0 || $ref!="")){
         $ret=$form->form_confirm($PHP_SELF.'?action=confirm_delete&id='.$id,$langs->trans('DeleteTimecardwhitelist'),$langs->trans('ConfirmDeleteTimecardwhitelist'),'confirm_delete', '', 0, 1);
 ...
 //       
        ...
        if($edit==0 && $new==0){ //show tabs
            $head=Timecardwhitelist_prepare_head($object);
            dol_fiche_head($head,'card',$langs->trans('Timecardwhitelist'),0,'timecard@timecard');            
        }else{
            print_fiche_titre($langs->trans('Timecardwhitelist'));
        }
...
// show the field date_end
	...
	print '<div class="center">';
        ...
        if($new==1){
            ...
            if (empty($reshook) && ($user->admin || $user->id == $object->user))
            {
                ...
                // Boutons d'actions
                //if($user->rights->Timecardwhitelist->edit)
                //{
                    print '<a href="'.$PHP_SELF.'?id='.$id.'&action=edit" class="butAction">'.$langs->trans('Update').'</a>';
                //}
                
                //if ($user->rights->Timecardwhitelist->delete)
                //{
  ...
  /*
        ...
        print_fiche_titre($langs->trans('Timecardwhitelist'));
        $head=Timecardwhitelist_prepare_head($object);
        dol_fiche_head($head,'info',$langs->trans("Timecardwhitelist"),0,'timecard@timecard');            
	...
	$object->fetch_thirdparty();
        ...
        $head=Timecardwhitelist_prepare_head($object);
        dol_fiche_head($head,'documents',$langs->trans("Timecardwhitelist"),0,'timecard@timecard');            
	...
	//print "<tr><td>".$langs->trans("Company")."</td><td>".$object->client->getNomUrl(1)."</td></tr>";
        ...
        $modulepart = 'timecard';
        $permission = $user->rights->timecard->add;
        $param = '&id='.$object->id;
...
*/
    ...
    $sql.= ' FROM '.MAIN_DB_PREFIX.'timecard_whitelist as t';
    $sqlwhere='';
...
{
        $sqlcount='SELECT COUNT(*) as count FROM '.MAIN_DB_PREFIX.'timecard_whitelist as t';
        if(!empty($sqlwhere))
	...
	if (!empty($ls_date_end_year))	$param.='&ls_date_end_year='.urlencode($ls_date_end_year);
        ...
        print_barre_liste($langs->trans("Timecardwhitelist"),$page,$PHP_SELF,$param,$sortfield,$sortorder,'',$num,$nbtotalofrecords);
...
//Search field fordate_end
    ...
    print ' '.$langs->trans('Timecardwhitelist')."</a>\n";
...
function Timecardwhitelist_prepare_head($object)
{
