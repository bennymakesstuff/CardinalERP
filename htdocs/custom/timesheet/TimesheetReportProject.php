...
//query to get the project where the user have priviledge; either project responsible or admin
...
$sql.=' ORDER BY pjt.ref';  // added by PJR
...
if ($projectSelectedId   &&!empty($dateStart))
{
    ...
    }elseif($projectSelectedId=='-998'){  // added by PJR
        foreach($projectList as $project){
//			print '<br/>$project->ref: '.$project->ref;
			
			if(substr($project->ref,0,2)=='FR'){
				$querryRes.=$project->getHTMLreport($short,
			   dol_print_date($dateStart,'day').'-'.dol_print_date($dateEnd,'day'),
				$conf->global->TIMESHEET_DAY_DURATION,$exportfriendly);
			}
...
//    if($user->admin){
        $Form.='<option value="-998" '.(($projectSelectedId=="-998")?"selected":'').' >'.$langs->trans('All FR Projects').'</option>'."\n";
...
// Select mode
$Form.= '<td><input type="radio" name="mode" value="TUD" '.($mode=='TUD'?'checked':'');
$Form.='> '.$langs->trans('Task').' / '.$langs->trans('User').' / '.$langs->trans('Date').'<br>';
$Form.= '<input type="radio" name="mode" value="TDU" '.($mode=='TDU'?'checked':'');
$Form.='> '.$langs->trans('Task').' / '.$langs->trans('Date').' / '.$langs->trans('User').'<br>';
$Form.= '<input type="radio" name="mode" value="UTD" '.($mode=='UTD'?'checked':'');
