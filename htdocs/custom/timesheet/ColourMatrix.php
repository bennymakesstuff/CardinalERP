...
*/
			...
			*/
			...
			$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."projet_extrafields as pc on p.rowid = pc.fk_object";
...
*/
			print '<tr class="liste_titre">';
			print '<th align = "center" width= "50">Make </td>';
			for ($i = 0 ; $i < $numcols; $i++)
			{
				$textcol = '<font>';
				if ($colcodes[$i]->text_col == 1) $textcol = '<font color = "white">';
				print '<td class="cmatrix" align="center" bgcolor="'.$colcodes[$i]->hex_code.'" width= "50">'.$textcol.$colcodes[$i]->colourname.'</font></td>';
			}
			print '<td class="cmatrix" align="center" width= "50">Needs Colour allocated</td>';
...
//				print '<td valign = "middle" colspan = "'.($numcols + 3).'">'.img_picto($carmakes[$j]->makename, '/custom/autos/img/icons/'.$carmakes[$j]->make_logo,'',1,0,0).'<br/><b>'.$carmakes[$j]->makename.'</b></td>';
				if ($carmakes[$j]->makename)
					print '<td class = "cmatrix" align = "center"><img src="/custom/autos/img/icons/'.$carmakes[$j]->make_logo.'" border="0" title="'.$carmakes[$j]->makename.'" width = "'.$mlwidth.'"><br/><b>'.$carmakes[$j]->makename.'</b></td>';
				else
					print '<td class = "cmatrix" align = "center"><b>Needing Make to be allocated</b></td>';
...
*/
				...
				for ($k = 0 ; $k <= $numcols ; $k++)
				{
					if ($colmakes[$j][$k]->colcount > 0)
					{
						...
						{
							$numprojects = $db->num_rows($resql);
							print '<td class="nowrap cmatrix" align="center" bgcolor="'.$colmakes[$j][$k]->hex_code.'">';
							for ($i = 0 ; $i < $numprojects; $i++)
								{
