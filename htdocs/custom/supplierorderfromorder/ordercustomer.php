...
dol_include_once("/custom/parts/class/ordlink.class.php");
...
$conf->liste_limit = 1000; // No pagination on this screen
...
if (in_array($action, array('valid-propal', 'valid-order') )) {
    ...
    if ($linecount > 0) {
        ...
        foreach ($suppliers as $idsupplier => $supplier) {
			...
			// If a draft status order already exists and the option SOFO_CREATE_NEW_SUPPLIER_ODER_ANY_TIME
			if($obj && !$conf->global->SOFO_CREATE_NEW_SUPPLIER_ODER_ANY_TIME) {
				...
				// We check that there is not already a link between the sales order and the supplier order in the element_element table.
				// If there are none, we add it, otherwise we do not add it
				...
				$id++; // $id must be filled in all cases so that the message 'Your orders have been generated' is displayed
				$newCommande = false;
...
//var_dump($line);
				...
				// We add a line only if an "updateline ()" has not been done and if the desired quantity is greater than zero
				...
				if(!$done) {
					...
					$lastid = $order->addline( // PJR mod
						...
						// PJR mod - Get link data from Sale Order to Purchase Order
						$ordlink->fk_object=$lastid;
						$ordlink->fk_commandedet=$line->fk_commandedet;
						$ordlink->create($user);
...
//         '<input type="hidden" name="show_stock_no_need" value="' . GETPOST('show_stock_no_need'). '">'.
         ...
         '<div style="text-align:right">'; // PJR mod
...
print '	  </div>'.
    ...
    if($conf->global->SOFO_USE_DELIVERY_TIME) {
            ...
            $colspan = empty($conf->global->FOURN_PRODUCT_AVAILABILITY) ? 1 : 2;
...
*/ //PJR mod
	...
	/* We only display "Physical Stock" when it is indeed the case:
	 * - If one is in the case of an OF with the necessary products
	 * - If we use the virtual stocks (either with the Dolibarr global conf or the module config) or that we use a time range for the need or that we do not take into account the customer orders
	 */
...
//PJR mod   	if (!empty($conf->global->FOURN_PRODUCT_AVAILABILITY)) print_liste_field_titre($langs->trans("Availability"));
...
// PJR mod    print '<th class="liste_titre" >&nbsp;</th>';  // PJR mod

    print '</tr>';  // PJR mod
	
/*    
    print '</tr>'.
	    // Lines of the filter fields
           ...
           if(!$justOFforNeededProduct) {
                ...
                if($week_to_replenish>0) {
    				...
    				if(!$conf->global->STOCK_CALCULATE_ON_SUPPLIER_VALIDATE_ORDER || $conf->global->SOFO_USE_VIRTUAL_ORDER_STOCK) {
						...
						//Query that retrieves the sum of qty broken down for partially received order
			...
			// We look if there is a current price request for this product
			$TDemandes = array();
			...
			if($conf->askpricesupplier->enabled) {
          	...
          	// The quantity to order corresponds to the desired stock on the product added to the desired quantity in the order :
...
/*			if($stocktobuy<=0 && $prod->ref!='A0000753') {
			...
			if($conf->of->enabled) {
				...
				/* If I have OF I want to know how much it costs me */
...
//$_REQUEST['DEBUG']=true;
			...
			if(strpos($objp->lineid, '@') === false) { // Single line of origin
				$lineid = $objp->lineid;
			}
			...
			print '</td>
                 ...
                 (!empty($TDemandes) ? $form->textwithpicto($prod->getNomUrl(1), 'Current price request(s) :<br />'.implode(', ', $TDemandes), 1, 'help') : $prod->getNomUrl(1)).
                 '</td>'.
                 '<td>' . $objp->label . '</td>';
...
*/					$prod->load_stock();
	  ...
	  if(empty($fk_commande)) $TCachedProductId[] = $prod->id; //cached
...
function _prepareLine($i,$actionTarget = 'order')
{
	...
	if(!GETPOST('tobuy_free'.$i)){
		...
		if(! empty($lineid)) {
			...
			if(empty($commandeline->id) && ! empty($commandeline->rowid)) {
				$commandeline->id = $commandeline->rowid; // Not positioned by OrderLine :: fetch () so the fetch_optionals () fair ...
			}
	...
	else
	{
		...
		$price = 0;
		$price = price2num(GETPOST('price_free'.$i, 'int'));
		...
		$line->subprice = $price;
		$line->total_ht = 0;
//		$line->total_ht = $price * $qty; // PJR TODO was causing a non-numeric errorlog
		$tva = $line->tva_tx / 100;
		$line->total_tva = $line->total_ht * $tva;
		$line->total_ttc = $line->total_ht + $line->total_tva;
		//$line->ref_fourn = $obj->ref_fourn;
		...
		$line->array_options = $array_options;
		$line->fk_commandedet = $commandeline->rowid;
