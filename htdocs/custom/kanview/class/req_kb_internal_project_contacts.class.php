...
$sql .= " " . "" . LLX_ . "user.login AS login";
		...
		$sql .= " FROM ";
		$sql .= "" . LLX_ . "element_contact AS t   left join " . LLX_ . "projet on t.element_id = " . LLX_ . "projet.rowid   left join " . LLX_ . "c_type_contact on t.fk_c_type_contact = " . LLX_ . "c_type_contact.rowid   left join " . LLX_ . "user on t.fk_socpeople = " . LLX_ . "user.rowid";

		// --------- WHERE
		$WHERE = "  " . LLX_ . "c_type_contact.element = 'project'  and  " . LLX_ . "c_type_contact.source = 'internal'";
		$WHERE = trim($WHERE);
		if (!empty($_WHERE)) {
			if ($_isNewWhere)
