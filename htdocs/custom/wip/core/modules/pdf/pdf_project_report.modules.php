...
//				$default_font_size = pdf_getPDFFontSize($outputlangs);	// Must be after pdf_getInstance
		        ...
		        $heightforfreetext= 5;//(isset($conf->global->MAIN_PDF_FREETEXT_HEIGHT)?$conf->global->MAIN_PDF_FREETEXT_HEIGHT:5);	// Height reserved to output the free text on last page // PJR mod - something broken in the global variable
...
//		$pdf->MultiCell($this->posxworkload-$this->posx_desc, 3, $outputlangs->transnoentities("Description"), 0, 'L');
		$pdf->MultiCell($this->posxworkload-$this->posx_desc, 3, "Description", 0, 'L');

		$pdf->SetXY($this->posx_hours, $tab_top+1);
//		$pdf->MultiCell($this->width_hours, 3, $outputlangs->transnoentities("Hours"), 0, 'R');
		$pdf->MultiCell($this->width_hours, 3, "Hours", 0, 'R');
	}
