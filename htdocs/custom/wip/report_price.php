...
/**
 *   	\file		htdocs/custom/wip/report_price.php
 *		\ingroup	wip
...
$langs->loadLangs(array('wip@wip', 'projects', 'products', 'companies', 'bills'));
...
$reshook=$hookmanager->executeHooks('doActions',$parameters,$socstatic,$action);	// Note that $action and $socstatic may have been modified by some hooks
...
$form		= new Form($db);
$socstatic	= new Societe($db);
$result = $socstatic->fetch($socid);

// wip_get_report_head parameters
$page_name	= "Price";
$page_title	= $langs->trans("ThirdParty") . '-' . $langs->trans('PriceByCustomer');
$help_url	= '';
$target		= '';
$tabactive	= GETPOST('tab')?GETPOST('tab'):'report_price';
...
{
	print '<tr><td class="titlefield">' . $langs->trans('Prefix') . '</td><td colspan="3">' . $socstatic->prefix_comm . '</td></tr>';
}

if ($socstatic->client) {
	print '<tr><td class="titlefield">';
	print $langs->trans('CustomerCode') . '</td><td colspan="3">';
	print $socstatic->code_client;
	if ($socstatic->check_codeclient() != 0)
		print ' <font class="error">(' . $langs->trans("WrongCustomerCode") . ')</font>';
	print '</td></tr>';
}

if ($socstatic->fournisseur) {
	print '<tr><td class="titlefield">';
	print $langs->trans('SupplierCode') . '</td><td colspan="3">';
	print $socstatic->code_fournisseur;
	if ($socstatic->check_codefournisseur() != 0)
...
if (! empty($conf->global->PRODUIT_CUSTOMER_PRICES)) {
	...
	if ($action == 'add_customer_price') {
		...
		print '<form action="' . $_SERVER["PHP_SELF"] . '?id='.$id.'&amp;socid=' . $socstatic->id . '" method="POST">';
		print '<input type="hidden" name="token" value="' . $_SESSION ['newtoken'] . '">';
		print '<input type="hidden" name="action" value="add_customer_price_confirm">';
		print '<input type="hidden" name="id" value="' . $id . '">';
		print '<input type="hidden" name="socid" value="' . $socstatic->id . '">';
		print '<table class="border" width="100%">';
		print '<tr>';
		print '<td>' . $langs->trans('Service') . '</td>';
		print '<td>';
		$serviceselect=new ReportDet($db);
		print img_object($langs->trans('ShowService'),'service').'  ';
		$serviceselect->selectServices('', 'prodid', $conf->global->WIP_SERVICE_CATEGORY, 0, '', '', 0); // function selectServices($selected='',$htmlname='serviceid',$category=695,$mode=0,$morecss='',$morewherefilter='',$useempty=0)
		print '</td>';
		print '</tr>';

		// VAT
		print '<tr><td>' . $langs->trans("VATRate") . '</td><td>';
		print $form->load_tva("tva_tx", /*$socstatic->tva_tx*/ 10, $mysoc, '', $socstatic->id, $socstatic->tva_npr, '', false, 1);
		print '</td></tr>';
		...
		print '</td>';
		print '<td>';
		print $form->selectPriceBaseType($socstatic->price_base_type, "price_base_type");
		print '</td>';
		print '</tr>';
		...
		print '</td><td>';
		if ($socstatic->price_base_type == 'TTC') {
			print '<input name="price" size="10" value="' . (price($socstatic->price_ttc)>0?price($socstatic->price_ttc):($conf->global->WIP_DEFAULT_SERVICE_PRICE?$conf->global->WIP_DEFAULT_SERVICE_PRICE:0)) . '">';
		} else {
			print '<input name="price" size="10" value="' . (price($socstatic->price)>0?price($socstatic->price):($conf->global->WIP_DEFAULT_SERVICE_PRICE?$conf->global->WIP_DEFAULT_SERVICE_PRICE:0)) . '">';
		}
		print '</td></tr>';
		...
		print $form->textwithpicto($text, $langs->trans("PrecisionUnitIsLimitedToXDecimals", $conf->global->MAIN_MAX_DECIMALS_UNIT), 1, 1);
		if ($socstatic->price_base_type == 'TTC') {
			print '<td><input name="price_min" size="10" value="' . (price($socstatic->price_min_ttc)>0?price($socstatic->price_min_ttc):($conf->global->WIP_DEFAULT_MINIMUM_PRICE?$conf->global->WIP_DEFAULT_MINIMUM_PRICE:0)) . '">';
		} else {
			print '<td><input name="price_min" size="10" value="' . (price($socstatic->price_min)>0?price($socstatic->price_min):($conf->global->WIP_DEFAULT_MINIMUM_PRICE?$conf->global->WIP_DEFAULT_MINIMUM_PRICE:0)) . '">';
		}
		print '</td></tr>';
		...
		if (count($prodcustprice->lines) > 0) {
			...
			print '<form action="' . $_SERVER["PHP_SELF"] . '?id=' . $socstatic->id . '" method="POST">';
			print '<input type="hidden" name="id" value="' . $socstatic->id . '">';
		...
		print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?id='.$id.'&amp;socid=' . $socstatic->id . '">' . $langs->trans("Ok") . '</a></div>';
		print "\n</div><br>\n";
	}
	else
	{
		...
		if ($user->rights->produit->creer || $user->rights->service->creer) {
			print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?id='.$id.'&amp;action=add_customer_price&amp;socid=' . $socstatic->id . '">' . $langs->trans("AddCustomerPrice") . '</a></div>';
		}
		print "\n</div>\n";
		...
		$option = '&search_prod=' . $search_prod . '&id=' . $socstatic->id;
