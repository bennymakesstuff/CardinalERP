...
/**
 *      \file       htdocs/projet/wip.php
 *      \ingroup    projet
 *		\brief      List page to show wip ordered for projects
 */
...
dol_include_once('/wip/class/ordlink.class.php');

// Load translation files required by the page
$langs->loadLangs(array('users', 'projects', 'companies', 'suppliers', 'compta', 'orders', 'wip@wip','other'));
...
if ($id == '' && $projectid == '' && $ref == '' && !($action == 'listall' || $action == 'builddoc'))
{
	dol_print_error('','Bad parameter action = '.$action);
	exit;
}
...
if ($action == 'listall' || $action == 'builddoc')
{
	if (! $sortfield) $sortfield='cd_project_ref, c_rowid, cd_rowid, cf_rowid';
...
//$hookmanager->initHooks(array('wiplist'));
...
{
	$title = $langs->trans('WIP for all Projects');
	llxHeader('',$title);

	$contentdesc = $langs->trans('This table presents all wip listed in Purchase Requisitions (PR) and Purchase Orders (PO) for all projects');
}
else  // Individual project
{
	$title = $langs->trans("WIP").' - '.$object->ref;
	llxHeader('',$title);

	$head=project_prepare_head($object);
	dol_fiche_head($head, 'wip', $langs->trans("Project"), -1, ($object->public?'projectpub':'project'));

	$contentdesc = $langs->trans('This table presents the wip listed in Purchase Requisitions (PR) and Purchase Orders (PO) for Project').' - '.$object->ref;
}
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/wip/ordlink_card.php?id='.$id);
	exit;
}
*/
...
print '<!-- List of wip for project(s) -->';
/*
...
$urlsource=$_SERVER["PHP_SELF"]./*'?action=listall'.*/'#builddoc';
$genallowed=1;
$delallowed=1;
$modelpdf=wiplist;
