...
// 4.Update hours
	//$billperiod = $conf->global->WIP_TIME_BLOCK;  //  Originally, to make code below a little more readable
	//$billperiod is now deprecated - now storing current billperiod in database table as it may be changed in settings by users at different times
...
//	$sql_4.= ', SUM(	CEIL(	(	CEIL(  (CEIL(  (tt.task_duration/60) / '.$billperiod.'  ) * '.$billperiod.')   / (60/100)  )/100	) * (100 - wrd1.discount_percent)   )/100	) AS dischours';
	$sql_4.= ', SUM(				CEIL(  (CEIL(  (tt.task_duration/60) / wrd1.time_block  ) * wrd1.time_block)   / (60/100)  )/100												) AS billhours';
	$sql_4.= ', SUM(	CEIL(	(	CEIL(  (CEIL(  (tt.task_duration/60) / wrd1.time_block  ) * wrd1.time_block)   / (60/100)  )/100	) * (100 - wrd1.discount_percent)   )/100	) AS dischours';
...
print '<br><br>Current $billperiod = '.$billperiod;
