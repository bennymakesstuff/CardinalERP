...
//		print '<label for="fk_user_author">' . $langs->trans("AuthorReport") . '</label> ';
	...
	if ($num > 0)
	{
		foreach ($servicessarray as $key=>$value)
		{
			...
			print '<td>' . dol_print_date($servicessarray[$key][pdate_price], "day") . '</td>';
...
if ($id > 0 || ! empty($ref))
{
		...
		if ($user->societe_id == 0 && $action != 'edit'&& $action != 'editline' && $action != 'delete')
		{
			...
			print '</div>Helloworld got to here 2731';	// End TabsAction
		}
...
print' Hello world got to here 2805';
//			print $formfile->showdocuments('wip_report', '', $filedir, $urlsource, $genallowed, $delallowed, $object->modelpdf, 1, 0, 0, 28, 0, '', '', '', $soc->default_lang);
			print $formfile->showdocuments('wip_report', $modulesubdir, $filedir, $urlsource, $genallowed, $delallowed, $object->modelpdf, 1, 0, 0, 28, 0, '', '', '', $soc->default_lang);
print' Hello world got to here 2808';
