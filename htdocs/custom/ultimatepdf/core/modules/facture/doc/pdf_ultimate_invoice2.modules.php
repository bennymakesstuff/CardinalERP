...
/**
 *	\file       htdocs/custom/ultimatepdf/core/modules/facture/doc/pdf_ultimate_lcr.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate invoices from ultimate_lcr model
 */
...
/**
 *	\class      pdf_ultimate_invoice2
 *	\brief      Class to manage PDF invoice pdf_ultimate_invoice2 template
 */

class pdf_ultimate_invoice2 extends pdf_ultimate_invoice1
{
	...
	public function __construct($db)
	{
		...
		$this->name = "ultimate_invoice2";
		$this->description = $langs->trans('PDFUltimate_invoice2Description');
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					{
						$up_before_remise_percent = pdf_getlineupexcltax($object, $i, $outputlangs, $hidedetails);
						$pdf->SetXY ($this->posxup, $curY);
						$pdf->MultiCell($this->posxdiscount-$this->posxup-0.8, 3, $up_before_remise_percent, 0, 'R', 0);
					}
					...
					else
					{
						...
						{
							$up_after = price2num((float)(100 - $remise_percent) * price2num($up_before_remise_percent) / (float)100, 'MU');
							$pdf->SetXY ($this->posxupafter, $curY);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			if ($this->atleastonediscount)
			{
				...
				$pdf->MultiCell($this->posxupafter-$this->posxdiscount+1,6, $outputlangs->transnoentities("ReductionShort"), 0, 'C', 1);
			}
		}
		...
		if($conf->global->PRODUCT_USE_UNITS) 
		{
			...
			if (empty($hidetop)) 
			{
				...
				$pdf->MultiCell($this->postotalht - $this->posxunit +2, 6, $outputlangs->transnoentities("Unit"), '',
					'C', 1);
			}
		}    
