...
/**
 *	\file       htdocs/custom/core/modules/propale/pdf_ultimate_weight_propal1.modules.php
 *  \ingroup    propale
 *  \brief      Fichier de la classe permettant de generer les propales au modele ultimate_weight_propal1 
 */
 
dol_include_once('/ultimatepdf/core/modules/propale/doc/pdf_ultimate_propal1.modules.php');

/**
 *	Class to generate PDF proposal pdf_ultimate_weight_propal1
 */
class pdf_ultimate_weight_propal1 extends pdf_ultimate_propal1
{
    ...
    public function __construct($db)
    {
        ...
        $this->name = "ultimate_weight_propal1";
        $this->description = $langs->trans('PDFUltimate_weightPropal1Description');
        ...
        if ($conf->propal->dir_output)
        {
            ...
            if (file_exists($dir))
            {
				...
				if ($conf->global->ULTIMATE_DOCUMENTS_WITH_REF == "yes")
				{
					...
					{
						$this->posxref=$this->marge_gauche+$this->posxnumber;
				...
				{	
					$this->posxdesc=$this->marge_gauche+$this->posxnumber;
				}
                ...
                for ($i = 0 ; $i < $nblignes ; $i++)
                {
					...
					$pdf->MultiCell($this->posxdiscount-$this->posxweight-0.8, 3, $weight, 0, 'C');
					...
					if ($object->lines[$i]->remise_percent)
					{
						...
						$pdf->MultiCell($this->postotalht-$this->posxdiscount-0.8, 3, $remise_percent, 0, 'C');
					}
