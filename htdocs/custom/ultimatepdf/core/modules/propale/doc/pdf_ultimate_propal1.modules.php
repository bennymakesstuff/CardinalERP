...
/**
 *	\file       htdocs/custom/core/modules/propale/pdf_ultimate_propal1.modules.php
 *  \ingroup    propale
 *  \brief      Fichier de la classe permettant de generer les propales au modele ultimate_propal1 
 */
...
/**
 *	Class to generate PDF proposal pdf_ultimate_propal1
 */
class pdf_ultimate_propal1 extends ModelePDFPropales
{
	/**
	...
	*/
    ...
    public function __construct($db)
    {
		...
		$this->db = $db;
        $this->name = "ultimate_propal1";
        $this->description = $langs->trans('PDFUltimate_propal1Description');
	...
	public function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
        ...
        if ($conf->propal->dir_output)
        {
            ...
            if (file_exists($dir))
            {
				...
				$pagenb++;
				$this->_pagehead($pdf, $object, 1, $outputlangs, $titlekey="CommercialProposal");
                ...
                for ($i = 0 ; $i < $nblignes ; $i++)
                {
					...
					{	
						...
						//test régions
						/*$subregions=array('page' => $pdf->getPage(), 'xt' => $this->posxpicture, 'yt' =>  $curY, 'xb' => $this->posxpicture, 'yb' =>  $posYAfterImage, 'side' => 'R');
						$regions=$pdf->getPageRegions();
						print 'subregions='.var_export($subregions,true).'<BR>';
						print 'regions='.var_export($regions,true).'<BR>';
						
						if (count($regions)==0) {
							$regions=array($subregions);
						}else {
							array_push($regions,$subregions);
						}						
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if ($conf->global->ULTIMATE_GENERATE_DOCUMENTS_WITHOUT_VAT == "no" && empty($conf->global->ULTIMATE_SHOW_HIDE_VAT_COLUMN))
		{
			...
			if (empty($hidetop))
			{
				$pdf->SetXY($this->posxtva, $tab_top);
				$pdf->MultiCell($this->tva_width, 6, $outputlangs->transnoentities("VAT"), 0, 'C', 1);
			}
		}	
		...
		if($conf->global->PRODUCT_USE_UNITS) 
		{
			...
			if (empty($hidetop)) 
			{
				...
				$pdf->MultiCell($this->posxdiscount - $this->posxunit - 1, 6, $outputlangs->transnoentities("Unit"), '',
					'C', 1);
			}
		}
       ...
       $pdf->line($this->posxdiscount-1, $tab_top, $this->posxdiscount-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			...
			{
				$pdf->SetXY ($this->posxdiscount-2, $tab_top);
				$pdf->MultiCell($this->postotalht-$this->posxdiscount+2, 6, $outputlangs->transnoentities("ReductionShort"), 0, 'C', 1);
			}
		}
        ...
        if (!empty($conf->global->ULTIMATE_SHOW_LINE_TTTC))
		{
			if (empty($hidetop))
			{
				...
				$pdf->MultiCell(($this->page_largeur-$this->marge_droite)-$this->postotalht+2, 6, $outputlangs->transnoentities("TotalTTC"), 0, 'C', 1);
			}
		}
		else
		{
			...
			{
				$pdf->SetXY ($this->postotalht-2, $tab_top);
				$pdf->MultiCell(($this->page_largeur-$this->marge_droite)-$this->postotalht+2, 6, $outputlangs->transnoentities("TotalHT"), 0, 'C', 1);
			}
		}
    }
	 ...
	 *  @return	void
	 */
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey="CommercialProposal")
	{
		global $conf,$langs,$db,$dolibarr_main_data_root;
		...
		$posy = pdf_writeUltimateLinkedObjects($pdf, $object, $outputlangs, $posx, $posy, 100, 3, 'R', $default_font_size);
		...
		/*	$carac_emetteur='';
			...
			$arrayidcontact=$object->getIdContact('internal','SALESREPFOLL');
		 	...
		 	$carac_emetteur .= pdf_propal_build_address($outputlangs, $this->emetteur, $object->thirdparty);
			...
			{
				if ($conf->global->ULTIMATE_PDF_PROPAL_ADDALSOTARGETDETAILS == 1)
				{
					$hautcadre=68;
				}
				else
				...
				// Recipient address
				$carac_client=pdf_propal_build_address($outputlangs,$this->emetteur,$object->thirdparty,($usecontact?$object->contact:''),$usecontact,'target');
				...
				$pdf->SetAlpha($opacity);
				$pdf->RoundedRect($posx, $posy, $widthrecboxrecipient, $hautcadre*0.5, 2, $round_corner = '1111', 'FD', '', $bgcolor);
				$pdf->SetAlpha(1);
				$pdf->SetXY($posx+2,$posy);
			...
			elseif ($arrayidcontact=$object->getIdContact('external','CUSTOMER') && $object->getIdContact('external','SHIPPING'))
			{
				...
				$pdf->SetAlpha(1);
				$pdf->SetXY($posx+2,$posy);
				$pdf->MultiCell($widthrecboxrecipient-5,4, $outputlangs->transnoentities("TypeContact_propal_external_CUSTOMER"), 0, 'R');
			...
			elseif ($arrayidcontact=$object->getIdContact('external','BILLING'))
			{
				...
				$carac_client_name= pdfBuildThirdpartyName($thirdparty, $outputlangs);
				// Recipient address
				$carac_client=pdf_propal_build_address($outputlangs,$this->emetteur,$object->thirdparty,$object->contact,$usecontact,'target');
			...
			else
			{
				...
				$carac_client_name= pdfBuildThirdpartyName($thirdparty, $outputlangs);
				// Recipient address
				$carac_client=pdf_propal_build_address($outputlangs, $this->emetteur, $object->thirdparty, '', 0, 'target');
	        ...
	        $pdf->SetTextColorArray($textcolor);
	        $pdf->MultiCell($width, 5, $outputlangs->transnoentities("ProposalDate"), 0, 'C', false);
