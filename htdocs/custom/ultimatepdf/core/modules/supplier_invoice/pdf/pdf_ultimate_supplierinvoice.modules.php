	...
	*/
	...
	function write_file($object, $outputlangs='', $srctemplatepath='', $hidedetails=0, $hidedesc=0, $hideref=0)
	{
		...
		if ($conf->fournisseur->dir_output.'/facture')
		{
			...
			if (file_exists($dir))
			{
	            ...
	            $heightforfooter = $this->marge_basse + 5.5;	// Height reserved to output the footer (value include bottom margin)
				...
				$tab_top_newpage = (empty($conf->global->ULTIMATE_SUPPLIERINVOICES_PDF_DONOTREPEAT_HEAD)?$this->marge_haute+$logo_height+15:10);
				$tab_height = 130;
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					else
					{
						...
						if($page_current==$pageposbefore) 
						{
							...
							pdf_writelinedesc_ref($pdf,$object,$i,$outputlangs,$text_length-$curX,3,$curX,$curY,$hideref,$hidedesc,1,'label');
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if ($conf->global->ULTIMATE_GENERATE_DOCUMENTS_WITHOUT_VAT == "no")
		{
			...
			if (! $this->atleastoneratenotnull) // If no vat at all
			{
				...
				if (! empty($conf->global->FACTURE_LOCAL_TAX1_OPTION) && $conf->global->FACTURE_LOCAL_TAX1_OPTION=='localtax1on' && $object->total_localtax1>0)
				{
					...
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, price($object->total_localtax1, 0, $outputlangs, 0, -1, -1, $currency_code), 0, 'R', 1);
				}
				...
				if (! empty($conf->global->FACTURE_LOCAL_TAX2_OPTION) && $conf->global->FACTURE_LOCAL_TAX2_OPTION=='localtax2on' && $object->total_localtax2>0)
				{
					...
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, price($object->total_localtax2, 0, $outputlangs, 0, -1, -1, $currency_code), 0, 'R', 1);
				}
			}
			else
			{
		...
		if ($deja_regle > 0)
		{
			...
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($largcol2, $tab2_hl, price($object->total_ttc - $deja_regle, 0, $outputlangs, 0, -1, -1, $currency_code), $useborder, 'R', 1);
			$pdf->SetFont('','', $default_font_size - 1);
			$pdf->SetTextColorArray($textcolor);
		}
       ...
       if ($this->atleastonediscount)
		...
		if (!empty($conf->global->ULTIMATE_SHOW_LINE_TTTC))
		{
			if (empty($hidetop))
			{
				...
				$pdf->MultiCell(($this->page_largeur-$this->marge_droite)-$this->postotalht+2,6, $outputlangs->transnoentities("TotalTTCShort"), 0, 'R', 1);
			}
		}
		else
		{
			...
			{
				$pdf->SetXY ($this->postotalht-2, $tab_top);
				$pdf->MultiCell(($this->page_largeur-$this->marge_droite)-$this->postotalht+2, 6, $outputlangs->transnoentities("TotalHTShort"), 0, 'R', 1);
			}
		}
	}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($showaddress)
		{
			...
			if ($arrayidcontact=$object->getIdContact('external','BILLING') && $object->getIdContact('external','SHIPPING'))
			{
				...
				$pdf->MultiCell($widthrecbox-5,4, $outputlangs->transnoentities("TypeContact_invoice_supplier_external_BILLING"), 0, 'R');	
				...
				$pdf->MultiCell($widthrecbox-5,4, $outputlangs->transnoentities("TypeContact_invoice_supplier_external_SHIPPING"), 0, 'R');	
			...
			elseif ($arrayidcontact=$object->getIdContact('external','BILLING'))
			{
				...
				// Show sender contact address
				$pdf->SetAlpha($opacity);
				$pdf->RoundedRect($posx, $posy, $widthrecbox, $hautcadre, 2, $round_corner = '1111', 'FD', array(), $bgcolor);
				$pdf->SetAlpha(1);
				$pdf->SetXY($posx,$posy);		
				$pdf->MultiCell($widthrecbox,4, $outputlangs->transnoentities("TypeContact_invoice_supplier_external_BILLING"), 0, 'R');
			...
			{			
				...
				$pdf->RoundedRect($posx, $posy, $widthrecbox, $hautcadre, 2, $round_corner = '1111', 'FD', array(), $bgcolor);
				$pdf->SetAlpha(1);
				$pdf->MultiCell($widthrecbox,4, $outputlangs->transnoentities("TypeContact_invoice_supplier_external_SHIPPING"), 0, 'R');
			...
			// If SALESREPFOLL contact defined on invoice, we use it
			$usecontact=false;
			if ($arrayidcontact=$object->getIdContact('internal','SALESREPFOLL'))
			{
				if (count($arrayidcontact) > 0)
				{
					$object->fetch_user($arrayidcontact[0]);
					$contact_client = $outputlangs->transnoentities("Contact").": ".$outputlangs->convToOutputCharset($object->user->getFullName($outputlangs))."\n";
				}
				...
				if ($usecontact && !empty($conf->global->MAIN_USE_COMPANY_NAME_OF_CONTACT)) 
				{
					$thirdparty = $object->user->getFullName($outputlangs);
				} 
				else 
				{
					$thirdparty = $mysoc;
				...
				$carac_client=pdf_build_address($outputlangs,$this->emetteur,$mysoc,$object->user->getFullName($outputlangs),$usecontact,'target',$object);
				...
				$pdf->SetAlpha(1);
				$pdf->SetXY($posx+2,$posy);
				$pdf->MultiCell($widthrecbox-5,4, $outputlangs->transnoentities("BillTo"), 0, 'R');
				...
				$pdf->MultiCell($widthrecbox-5,4, $contact_client, 0, 'L');
