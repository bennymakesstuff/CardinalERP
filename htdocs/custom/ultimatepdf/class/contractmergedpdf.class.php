...
/**
 *  \file       ultimatepdf/class/contractmergedpdf.class.php
 *  \ingroup    ultimatepdf
 ...
 */
class Contractmergedpdf extends CommonObject
{
	var $db;							//!< To store db handler
	var $error;							//!< To return error code (or message)
	var $errors=array();				//!< To return several error codes (or messages)
	var $element='ultimatepdf_contract_merged_pdf';			//!< Id that identify managed objects
	var $table_element='ultimatepdf_contract_merged_pdf';		//!< Name of table without prefix where object is stored
    ...
    function create($user, $notrigger=0)
    {
        ...
        // Insert request
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."ultimatepdf_contract_merged_pdf(";
		...
		if (! $error)
        {
            $this->id = $this->db->last_insert_id(MAIN_DB_PREFIX."ultimatepdf_contract_merged_pdf");
    ...
    function fetch($id)
    {
        ...
        $sql.= " FROM ".MAIN_DB_PREFIX."ultimatepdf_contract_merged_pdf as t";
     ...
     *  Load object in memory from the database
     *
     *  @param	int		$contract_id    Id object
     *  @param	string	$lang  lang string id
     *  @return int          	<0 if KO, >0 if OK
     */
    function fetch_by_contract($contract_id, $lang='')
    {
    	global $langs,$conf;
    	...
    	$sql.= " FROM ".MAIN_DB_PREFIX."ultimatepdf_contract_merged_pdf as t";
    	$sql.= " WHERE t.fk_contrat = ".$contract_id;
    	...
    	if ($resql)
    	{
    		if ($this->db->num_rows($resql))
    		{
    			while($obj = $this->db->fetch_object($resql)) {
    				...
    				$line = new ContractmergedpdfcontractLine();
    ...
    function update($user=0, $notrigger=0)
    {
        ...
        // Update request
        $sql = "UPDATE ".MAIN_DB_PREFIX."ultimatepdf_contract_merged_pdf SET";
	...
	function delete($user, $notrigger=0)
	{
		...
		if (! $error)
		{
    		$sql = "DELETE FROM ".MAIN_DB_PREFIX."ultimatepdf_contract_merged_pdf";
    		$sql.= " WHERE rowid=".$this->id;
	 ...
	 *  @return	int					 <0 if KO, >0 if OK
	 */
	function delete_by_contract($user, $contract_id, $lang_id='',  $notrigger=0)
	{
		global $conf, $langs;
		$error=0;
		...
		{
			$sql = "DELETE FROM ".MAIN_DB_PREFIX."ultimatepdf_contract_merged_pdf";
			$sql.= " WHERE fk_contrat=".$contract_id;
	...
	function delete_by_file($user)
	{
		...
		{
			$sql = "DELETE FROM ".MAIN_DB_PREFIX."ultimatepdf_contract_merged_pdf";
			$sql.= " WHERE fk_contrat=".$this->fk_contrat." AND file_name='".$this->db->escape($this->file_name)."'";
	...
	function createFromClone($fromid)
	{
		...
		$object=new Contractmergedpdf($this->db);
...
/**
 * Class to manage contract merge of contract line
 */
class ContractmergedpdfcontractLine{
	var $id;
