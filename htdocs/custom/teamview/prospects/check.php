...
dol_include_once('/teamview/class/others/todo_prospect_comments.class.php');
...
// require_once DOL_DOCUMENT_ROOT . '/prospect/class/prospect.class.php';

$societe = new Societe($db);
$prospect = new prospects($db);

// $prospectorig = new Prospect($db);
// $prospectdet = new OrderLine($db);
...
$comments 	= new todo_prospect_comments($db);
...
if ($action == "oneProspect") {
	$prospect->fetchAll("", "", "", "", " AND rowid = ".$id_prospect);
	$item1 = $prospect->rows[0];

	$disabl = "disabled";
	if ($user->rights->projet->creer)
		$disabl = "";
	$prospect->fetch($item1->rowid);

	$html["ref"] = '<h2 class="title" id="'.$item1->rowid.'">'.$langs->trans("Code client").' : <span style="color: #5780ca;">'.$prospect->code_client.'</span></h2>';

	// $societe->fetch($item1->client);
	$html["tiers"] = $item1->nom;
	echo json_encode($html);
}

if ($action == "getallprospects") {
	$arr = [];
	...
	if (count($prospect->rows) > 0) {
	
		for ($i=0; $i < count($prospect->rows) ; $i++) {
			$item = $prospect->rows[$i];
			$prospect->fetch($item->rowid);


			$opacity2 = "1";
			$classNoComment = "classNoComment";
			$tot_cmnt = 0;
			$comments->fetchAll("", "", "", "", " AND id_prospect = ".$item->rowid);
			...
			if($item->fk_statut == 0){
				...
				$resql = $prospect->db->query($sql);
				if ($resql) {
					while ($obj = $prospect->db->fetch_object($resql)) {
			...
			$eeee = "";
			if ($fk_typent != "-")
				$eeee = '<span class="amount" title="'.$langs->trans("Type_du_tiers").'">'.$fk_typent.'</span>';
			...
			$societe->fetch($item->rowid);
			$societe->fetch($item->fk_soc);

			$popupinfo = "";
			$popupinfo .= '<b>'.$langs->trans("Réf").'.</b> : '.$societe->code_client   .'<br>';
			$popupinfo .= '<b>'.$langs->trans("Prospect").'</b> : '.$societe->nom   .'<br>';
			$popupinfo .= '<b>'.$langs->trans("Type_prospect").'</b> : '.$fk_typent   .'<br>';



			$content = '
				<div class="one_content '.$actf_cls.'" id="prospect_'.$item->rowid.'"  data-rowid="'.$item->rowid.'"  ondblclick="OpenProspectPop(this)" data-title="'.$popupinfo.'">
					<h4 id_prospect="'.$item->rowid.'"><a target="_blank" href="'.DOL_URL_ROOT.'/societe/card.php?socid='. $item->rowid .'">'.$prospect->code_client.'</a><span class="tier_span"> - '.$item->nom.'</span></h4>
					<input type="hidden" class="id_prospect" value="'.$item->rowid.'"/>
					'.$eeee.'
			...
			if($item->status == 0){
				$html["DESA"]["content"] .= $content;
			}else{
				if($item->fk_stcomm == 0){
					$html["JC"]["content"] .= $content;
				}
				elseif($item->fk_stcomm == 1){
					$html["AC"]["content"] .= $content;
				}
				elseif($item->fk_stcomm == 2){
					$html["CEC"]["content"] .= $content;
				}
				elseif($item->fk_stcomm == 3){
					$html["CR"]["content"] .= $content;
				}
				elseif($item->fk_stcomm == -1){
					$html["NPC"]["content"] .= $content;
				}
			}
			// 0 : Jamais contacté
			// 1 : À contacter
...
if ($action == "updateprospectetat") {
	$from_etat 		= $_POST['from_etat'];
	$to_etat 		= $_POST['to_etat'];
	$to_stcomm 		= $_POST['to_stcomm'];
	...
	if ($from_etat == 5) {
		...
		$isvalid = $prospect->update($id_prospect, $data);
	}
	elseif($to_etat == 5){
		$data = array(
			'status' => 	0
...
if ($action == "getallcomments") {
	$comments->fetchAll("DESC", "created_at", "", "", " AND id_prospect = ".$id_prospect);
	$content = "";
	...
	if (count($comments->rows) > 0) {
		for ($i=0; $i < count($comments->rows) ; $i++) {
			...
			$dire = DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/prospects/'.$item->rowid.'/';
			if (file_exists($dire)){
			$images = scandir($dire);
			$content .= '<div class="files_joints"><ul class="list_joints">';
			$dire = DOL_MAIN_URL_ROOT.$customtxt.'/teamview/files_commentaire/prospects/'.$item->rowid.'/';
			...
			$content .= '
						<div class="commentaire_txt_input" style="display:none;">
							<textarea class="textarea_comment prospect_comment_edit" rows="4" onkeyup="comment_change_edit(this)"></textarea>
...
if ($action == "create_comment") {
	...
	if(isset($_FILES['files'])) {  
		$dire_file = DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/prospects/'.$id.'/';
...
if ($action == "update_comment") {
    ...
    $dire = DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/prospects/'.$id_comment.'/';
    if($files_deleted){
...
if ($action == "delete_comment") {
	$id_comment = $_POST['id_comment'];
	$dire = DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/prospects/'.$id_comment.'/';
	$files = glob(DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/prospects/'.$id_comment.'/*');
	if (file_exists($dire)){
...
if ($action == "upload_file") {
	$id_comment 	= $_POST['id_comment'];
	if(isset($_FILES['files'])) {  
		$dire_file = DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/prospects/'.$id_comment.'/';
	    mkdir($dire_file, 0777, true);
	    $names = array();
		foreach ($_FILES["files"]["name"] as $key => $value) {
	        if ($error == UPLOAD_ERR_OK) {
	...
	// {  
	//     $uploads_dir =  DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/prospects/'.$id_comment.'/';
...
if ($action == "getchildtasks") {
	...
	if (count($tasksarray) > 0)
	{
		...
		$j=0; $level=0;
		$html .= $teamview->projectLinesa($j, $id_prospect, $tasksarray, $level, true, 0, $tasksrole, $id_projet, 1, $id_projet);
	}
	else
	{
