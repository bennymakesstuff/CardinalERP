...
function load_all_prospects(){
	...
	var data = {
		...
		'offset' : offset,
		'filter' : filter,
		'action' : "getallprospects"
	};
	$.ajax({
		type: "POST",
		url: "<?php echo dol_buildpath('/teamview/prospects/check.php',2); ?>",
		data: data, 
		dataType: 'json',
...
function OpenProspectPop($t){
	var id_prospect = $($t).find('.id_prospect').val();
	var data = {
	...
	$.ajax({
		...
		success: function(found){
			if (found) {
				// console.log("Prospect found : "+found);
				$("#prospect_title").html(found["ref"]);
				$("#prospect_tiers").html(found["tiers"]);
	...
	$(".one_content").draggable({
  	...
  	$(".scroll_div").droppable({
		...
		drop: function (event, ui) {
			...
			var to_stcomm = $(this).parent().parent().data("stcomm");

			var from = ui.draggable.parent().parent().parent().attr("id");
			var to = $(this).parent().parent().attr("id");
			var id_prospect = ui.draggable.data("rowid");
...
function get_files_joints() {
	var id_prospect 	= $("#prospect_title").find('.title').attr('id');
...
function to_delete_file($t, e, cmntId){
    ...
    if ($('.one_comment#comment_'+cmntId).find('.prospect_comment_edit').val() == "")
...
function progress_tasks_change(){
	var all100 = true;
	$('#tablelines > tbody  > tr .prospect_avanc').each(function() {
		tot = $(this).find('select option:selected').val();
		if (tot < 100)
			all100 = false;
	});
	if (all100){
		$("#prospect_avance select").val(100);
		$("#prospect_avance select").trigger('change');
		// createorupdatetask();
	}
...
function update_comment($t) {
	...
	var id_comment 	= $($t).parent().parent().find(".id_comment").val();
	var comment 	= $($t).parent().find(".prospect_comment_edit").val();
...
function create_comment($t) {
	var id_prospect 	= $("#prospect_title").find('.title').attr('id');
	var comment 	= $("#prospect_comment").val();
