...
function load_all_factures(){
	...
	var data = {
		...
		'offset' : offset,
		'filter' : filter,
		'action' : "getallfactures"
	};
	$.ajax({
		type: "POST",
		url: "<?php echo dol_buildpath('/teamview/factures/check.php',2); ?>",
		data: data, 
		dataType: 'json',
...
function OpenFacturePop($t){
	var id_facture = $($t).find('.id_facture').val();
	var data = {
	...
	$.ajax({
		...
		success: function(found){
			if (found) {
				// console.log("Facture found : "+found);
				$("#facture_title").html(found["ref"]);
				$("#facture_tiers").html(found["tiers"]);
	...
	$(".one_content").draggable({
  	...
  	$(".scroll_div").droppable({
		...
		drop: function (event, ui) {
			...
			var id_facture = ui.draggable.data("rowid");

			var arr = [];
			if (to == "DRAFT")
				arr = [];
			else if (to == "VALIDATED")
				arr = ["DRAFT","PAYEE","ABONDONNEE"];
			else if (to == "PAYEE")
				arr = ["CPAYEE"];
			else if (to == "ABONDONNEE")
				arr = ["VALIDATED","RETARD"];
			...
			if (to !== from && permis == 1) {
				...
				if (arr.indexOf(from) >= 0){
					if(from == "DRAFT" && ui.draggable.data("lines") == 0){
						msg = "<?php echo trim(addslashes($langs->trans("factureVide"))); ?>";
						$.jnotify(msg, "warning", true, { remove: function (){} } );
					}else{
						var data = {
							'id_facture' : id_facture,
							'from_etat' : from_etat,
							'to_etat' : to_etat,
							'action' : "updatefactureetat"
						};
						$.ajax({
							type: "POST",
							url: "<?php echo dol_buildpath('/teamview/factures/check.php',2); ?>",
							...
							success: function(found){
								...
								if (from == "DRAFT")
									load_all_factures();
								if ((from == "PAYEE" || from == "ABONDONNEE" ) && to == "VALIDATED")
									load_all_factures();

								countEachColumnNumbers();
							}
						});
						// $("#"+to+" .scroll_div").removeClass("dropped");
					...
					var msg = "";
					if (to == "DRAFT")
						msg = "<?php echo trim(addslashes($langs->trans("rollBackToDraft"))) ?>";
					else if (from == "DRAFT" && to == "ABONDONNEE")
						msg = "<?php echo trim(addslashes($langs->trans("firstValidate"))); ?>";
					else
						msg = "<?php echo trim(addslashes($langs->trans("noAutorAction"))); ?>";
  	...
  	// $('.scroll_div#scroll_5 .one_content,.scroll_div#scroll_6 .one_content').draggable( "option", "disabled", true );
}
...
function get_files_joints() {
	var id_facture 	= $("#facture_title").find('.title').attr('id');
...
function to_delete_file($t, e, cmntId){
    ...
    if ($('.one_comment#comment_'+cmntId).find('.facture_comment_edit').val() == "")
...
function progress_tasks_change(){
	var all100 = true;
	$('#tablelines > tbody  > tr .facture_avanc').each(function() {
		tot = $(this).find('select option:selected').val();
		if (tot < 100)
			all100 = false;
	});
	if (all100){
		$("#facture_avance select").val(100);
		$("#facture_avance select").trigger('change');
		// createorupdatetask();
	}
...
function update_comment($t) {
	...
	var id_comment 	= $($t).parent().parent().find(".id_comment").val();
	var comment 	= $($t).parent().find(".facture_comment_edit").val();
...
function create_comment($t) {
	var id_facture 	= $("#facture_title").find('.title').attr('id');
	var comment 	= $("#facture_comment").val();
	...
	formData = new FormData();
	// formData.append( 'id_comment', id_comment );
	formData.append( 'id_facture', id_facture );
