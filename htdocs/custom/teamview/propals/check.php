...
dol_include_once('/teamview/class/others/todo_propal_comments.class.php');
...
dol_include_once('/teamview/class/others/propals.class.php');
require_once DOL_DOCUMENT_ROOT . '/comm/propal/class/propal.class.php';

$societe = new Societe($db);
$propal = new propals($db);

$propalorig = new Propal($db);
...
$comments 	= new todo_propal_comments($db);
...
if ($action == "onePropal") {
	$propal->fetchAll("", "", "", "", " AND rowid = ".$id_propal);
	$item1 = $propal->rows[0];

	$disabl = "disabled";
	if ($user->rights->projet->creer)
		$disabl = "";
	$propal->fetch($item1->rowid);

	$html["ref"] = '<h2 class="title" id="'.$item1->rowid.'">'.$langs->trans("ref_propal").' : <span style="color: #5780ca;">'.$propal->getNomUrl(0,"","",1).'</span></h2>';
...
if ($action == "getallpropals") {
	...
	$propal->fetchAll($sortorder, $sortfield, $limit, $offset, $filter);

	if (count($propal->rows) > 0) {
		$arr["DRAFT"] = $arr["VALIDATED"] = $arr["SIGNED"] = $arr["NOTSIGNED"] = $arr["BILLED"] = "";

		for ($i=0; $i < count($propal->rows) ; $i++) {
			$item = $propal->rows[$i];
			$propal->fetch($item->rowid);
			$opacity2 = "1";
			$classNoComment = "classNoComment";
			$tot_cmnt = 0;
			$comments->fetchAll("", "", "", "", " AND id_propal = ".$item->rowid);
			...
			$popupinfo = "";
			$popupinfo .= '<b>'.$langs->trans("Réf").'.</b> : '.$propalorig->ref   .'<br>';
			$popupinfo .= '<b>'.$langs->trans("Client").'</b> : '.$societe->nom   .'<br>';
			$popupinfo .= '<b>'.$langs->trans("Date_proposition").'</b> : '. dol_print_date($propalorig->date,'day')   .'<br>';
			$popupinfo .= '<b>'.$langs->trans("Date_livraison").'</b> : '. dol_print_date($propalorig->date_livraison,'day')   .'<br>';
			$popupinfo .= '<b>'.$langs->trans("Fin_validité").'</b> : '. dol_print_date($propalorig->fin_validite,'day')   .'<br>';
			$popupinfo .= '<b>'.$langs->trans("Total_TTC").'</b> : '. number_format($propalorig->total_ttc,2,","," ") .' '.$langs->getCurrencySymbol($conf->currency).'<br>';
			...
			$content = '
				<div class="one_content '.$actf_cls.'" id="propal_'.$item->rowid.'"  data-rowid="'.$item->rowid.'"  ondblclick="OpenPropalPop(this)" data-title="'.$popupinfo.'">
					<h4 id_propal="'.$item->rowid.'">'.$propal->getNomUrl(0,"","",1).'<span class="tier_span"> - '.$soc.'</span></h4>
					<input type="hidden" class="id_propal" value="'.$item->rowid.'"/>
					<span class="amount" title="'.$langs->trans("Total_TTC").'">'.number_format($item->total,2,","," ").' '.$langs->getCurrencySymbol($conf->currency).' </span>
			...
			if($item->fk_statut == 2){
				$html["SIGNED"]["content"] .= $content;
			}
			if($item->fk_statut == 3){
				$html["NOTSIGNED"]["content"] .= $content;
			}
			if($item->fk_statut == 4){
				$html["BILLED"]["content"] .= $content;
			}
		}
	}
...
if ($action == "updatepropaletat") {
	$from_etat 		= $_POST['from_etat'];
	$to_etat 		= $_POST['to_etat'];

	$propalorig->fetch($id_propal);
	// print_r($propalorig);
	// die();
	if ($to_etat == 1) {
		if ($propalorig->statut == 0) {
			$result = $propalorig->valid($user);
		}else{
			$result = $propalorig->reopen($user, 1);
		}
	}else{
		if ($to_etat < $propalorig->statut) {
			// if ($to_etat == 0 && $propalorig->statut == 1) {
			// 	$object->set_draft($user);
			// }
			$result = $propalorig->reopen($user, $to_etat);
		}else{
			$result = $propalorig->cloture($user, $to_etat,"");
		}
	}
	$html = "true";
	echo json_encode($html);
...
if ($action == "getallcomments") {
	$comments->fetchAll("DESC", "created_at", "", "", " AND id_propal = ".$id_propal);
	$content = "";
	...
	if (count($comments->rows) > 0) {
		for ($i=0; $i < count($comments->rows) ; $i++) {
			...
			$dire = DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/propales/'.$item->rowid.'/';
			if (file_exists($dire)){
			$images = scandir($dire);
			$content .= '<div class="files_joints"><ul class="list_joints">';
			$dire = DOL_MAIN_URL_ROOT.$customtxt.'/teamview/files_commentaire/propales/'.$item->rowid.'/';
			...
			$content .= '
						<div class="commentaire_txt_input" style="display:none;">
							<textarea class="textarea_comment propal_comment_edit" rows="4" onkeyup="comment_change_edit(this)"></textarea>
			...
			$content .= '			<div class="one_file">
							...
							<button class="comment_btn update_comment button button_save_" onclick="update_comment(this);">Enregistrer</button> 
							<span class="cancel_cmt" onclick="cancel_cmt(this);" title="Annuler"><i class="fa fa-times"></i></span>
						</div>';
			}
	$content .= '</div>';
...
if ($action == "create_comment") {
	...
	if(isset($_FILES['files'])) {  
		$dire_file = DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/propales/'.$id.'/';
...
if ($action == "update_comment") {
    ...
    $dire = DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/propales/'.$id_comment.'/';
    if($files_deleted){
...
if ($action == "delete_comment") {
	$id_comment = $_POST['id_comment'];
	$dire = DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/propales/'.$id_comment.'/';
	$files = glob(DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/propales/'.$id_comment.'/*');
	if (file_exists($dire)){
...
if ($action == "upload_file") {
	$id_comment 	= $_POST['id_comment'];
	if(isset($_FILES['files'])) {  
		$dire_file = DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/propales/'.$id_comment.'/';
	    mkdir($dire_file, 0777, true);
	    $names = array();
		foreach ($_FILES["files"]["name"] as $key => $value) {
	        if ($error == UPLOAD_ERR_OK) {
	...
	// {  
	//     $uploads_dir =  DOL_DOCUMENT_ROOT.$customtxt.'/teamview/files_commentaire/propales/'.$id_comment.'/';
...
if ($action == "getchildtasks") {
	...
	if (count($tasksarray) > 0)
	{
		...
		$j=0; $level=0;
		$html .= $teamview->projectLinesa($j, $id_propal, $tasksarray, $level, true, 0, $tasksrole, $id_projet, 1, $id_projet);
	}
	else
	{
