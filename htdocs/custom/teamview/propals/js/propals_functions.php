...
function load_all_propals(){
	...
	var data = {
		...
		'offset' : offset,
		'filter' : filter,
		'action' : "getallpropals"
	};
	$.ajax({
		type: "POST",
		url: "<?php echo dol_buildpath('/teamview/propals/check.php',2); ?>",
		data: data, 
		dataType: 'json',
...
function OpenPropalPop($t){
	var id_propal = $($t).find('.id_propal').val();
	var data = {
		'id_propal' : id_propal,
		'action' : "onePropal"
	};
	$.ajax({
		...
		success: function(found){
			if (found) {
				// console.log("Propal found : "+found);
				$("#propal_title").html(found["ref"]);
				$("#propal_tiers").html(found["tiers"]);
	...
	$(".one_content").draggable({
  	...
  	$(".scroll_div").droppable({
		...
		drop: function (event, ui) {
			...
			var id_propal = ui.draggable.data("rowid");

			var arr = [];
			if (to == "DRAFT")
				arr = [];
			else if (to == "VALIDATED")
				arr = ["DRAFT","SIGNED","NOTSIGNED","BILLED"];
			else if (to == "SIGNED")
				arr = ["VALIDATED","NOTSIGNED","BILLED"];
			else if (to == "NOTSIGNED")
				arr = ["VALIDATED","SIGNED","BILLED"];
			else if (to == "BILLED")
				arr = ["VALIDATED","SIGNED"];
			...
			if (to !== from && permis == 1) {
				...
				if (arr.indexOf(from) >= 0){
					...
					$.ajax({
						...
						dataType: 'json',
						success: function(found){
							$.jnotify('<?php echo trim(addslashes($langs->trans("updatedSuccessfuly"))); ?>',
								"500",
								false,
								{ remove: function (){} } );
							if (from == "DRAFT")
					...
					var msg = "";
					<?php
						$noauroll = trim(addslashes($langs->trans("rollBackToDraft")));
						$noaufiv = trim(addslashes($langs->trans("firstValidate")));
						$noaufis = trim(addslashes($langs->trans("firstSign")));
					?>
					if (to == "DRAFT")
						msg = "<?php echo $noauroll; ?>";
					else if (to == "SIGNED" || to == "NOTSIGNED")
						msg = "<?php echo $noaufiv; ?>";
					else if (to == "BILLED"){
						if (from == "DRAFT")
							msg = "<?php echo $noaufiv; ?>";
						else if (from == "NOTSIGNED")
							msg = "<?php echo $noaufis; ?>";
					}
					if (msg !== ""){
...
function get_files_joints() {
	var id_propal 	= $("#propal_title").find('.title').attr('id');
...
function to_delete_file($t, e, cmntId){
    ...
    if ($('.one_comment#comment_'+cmntId).find('.propal_comment_edit').val() == "")
...
function progress_tasks_change(){
	var all100 = true;
	$('#tablelines > tbody  > tr .propal_avanc').each(function() {
		tot = $(this).find('select option:selected').val();
		if (tot < 100)
			all100 = false;
	});
	if (all100){
		$("#propal_avance select").val(100);
		$("#propal_avance select").trigger('change');
		// createorupdatetask();
	}
...
function update_comment($t) {
	...
	var id_comment 	= $($t).parent().parent().find(".id_comment").val();
	var comment 	= $($t).parent().find(".propal_comment_edit").val();
...
function create_comment($t) {
	var id_propal 	= $("#propal_title").find('.title').attr('id');
	var comment 	= $("#propal_comment").val();
	...
	formData = new FormData();
	// formData.append( 'id_comment', id_comment );
	formData.append( 'id_propal', id_propal );
