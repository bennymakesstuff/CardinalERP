...
$modname = $langs->trans("etatpropals");
$var 				= true;
$form 				= new Form($db);
...
$nbrtotal = $propal->fetchAll($sortorder, $sortfield, 0, 0, $filter);
...
$morejs  = array("/teamview/js/jquery.slimscroll.min.js","/teamview/propals/js/propals.js","/includes/jquery/plugins/timepicker/jquery-ui-timepicker-addon.js");
llxHeader(array(), $modname,'','','','',$morejs,0,0);
// print_fiche_titre($modname);
$propal->fetchAll($sortorder, $sortfield, $limit, $offset, $filter);
$num = count($propal->rows)+1;
// echo "num : ".$num;
print '<form method="get" action="'.$_SERVER["PHP_SELF"].'">'."\n";
// print_barre_liste($modname, $page, $_SERVER["PHP_SELF"], $params, $sortfield, $sortorder, "", $num, $nbrtotal);
...
print '</td>';
print '<td>';
	print $propal->select_all_clients($srch_client,'srch_client',1,"rowid","nom",'','required="required"');
print '</td>';
print '</tr>';
print '</table>';
print '</div>';

// print '<span>'.$langs->trans("Client").'</span>';
// print '<label>'.$propal->select_all_clients($srch_client,'srch_client',1,"rowid","nom",'','required="required"').'</label>';
// print '<input type="submit" value="'.$langs->trans("Rechercher").'" class="butAction" />';
// print '</div>';
...
<div class="todo_content" style="display: flex;">
	...
	<div class="notsigned_div columns_ fifth_width" id="NOTSIGNED" data-etat="3">
		<div class="todo_titre"><b>'.$langs->trans("Non_signées").'</b><span class="filter_in_etat" id="nbr_NOTSIGNED"/></span></div>
		<div class="contents">
			<div class="scroll_div" id="scroll_4">
			</div>
		</div>
	</div>
	<div class="signed_div columns_ fifth_width" id="SIGNED" data-etat="2">
		<div class="todo_titre"><b>'.$langs->trans("Signées").'</b><span class="filter_in_etat" id="nbr_SIGNED"/></span></div>
		<div class="contents">
			<div class="scroll_div" id="scroll_3">
			</div>
		</div>
	</div>
	<div class="billed_div columns_ fifth_width" id="BILLED" data-etat="4">
		<div class="todo_titre"><b>'.$langs->trans("Facturées").'</b><span class="filter_in_etat" id="nbr_BILLED"/></span></div>
		<div class="contents">
...
print '</div><div class="hover_bkgr_fricc">
    ...
    <div class="windows_pop nc_pop">
        ...
        <div class="window-header">
	        ...
	        <div class="window-title" id="propal_title">
		        <h2 class="title"></h2>
	        </div>
	        <div>
	    		Tiers : <span id="propal_tiers"></span>
	        </div>
	        ...
	        <div>
	        	...
	        	<p style="margin-bottom: 0;"><textarea id="propal_comment" class="textarea_comment" rows="2" onkeyup="comment_change()" placeholder="'.$langs->trans("Écrivez_un_commentaire").'…"></textarea></p>
...
<?php
dol_include_once('/teamview/propals/js/propals_functions.php');
