...
$modname = $langs->trans("etatcommandeclient");
$var 				= true;
$form 				= new Form($db);
...
if (!empty($srch_debut)) {
	...
	$filter .= " AND DATE(date_commande) >= '".$debut."'";
}

if (!empty($srch_fin)) {
	$fin = explode("/",$srch_fin);
	$fin = $fin[2].'-'.$fin[1].'-'.$fin[0];
	$filter .= " AND DATE(date_commande) <= '".$fin."'";
}
// echo $filter;

$nbrtotal = $commande->fetchAll($sortorder, $sortfield, 0, 0, $filter);
...
$morejs  = array("/teamview/js/jquery.slimscroll.min.js","/teamview/commandes/js/commandes.js","/includes/jquery/plugins/timepicker/jquery-ui-timepicker-addon.js");
llxHeader(array(), $modname,'','','','',$morejs,0,0);
// print_fiche_titre($modname);
$commande->fetchAll($sortorder, $sortfield, $limit, $offset, $filter);
$num = count($commande->rows)+1;
...
print '<tr>';
print '<td>';
	print $langs->trans("Date_commande");
print '</td>';
...
print '</td>';
print '<td>';
	print $commande->select_all_clients($srch_client,'srch_client',1,"rowid","nom",'','required="required"');
print '</td>';
print '</tr>';
print '</table>';
print '</div>';
...
<div class="todo_content" style="display: flex;">
	...
	<div class="lnf_div columns_ fifth_width" id="LNF" data-etat="2">
		<div class="todo_titre"><b>'.$langs->trans("Livrées_non_facturées").'</b><span class="filter_in_etat" id="nbr_LNF"/></span></div>
		<div class="contents">
			<div class="scroll_div" id="scroll_3">
			</div>
		</div>
	</div>
	<div class="lf_div columns_ fifth_width" id="LF" data-etat="3">
		<div class="todo_titre"><b>'.$langs->trans('Livrées_facturées').'</b><span class="filter_in_etat" id="nbr_LF"/></span></div>
		<div class="contents">
			<div class="scroll_div" id="scroll_4">
			</div>
		</div>
	</div>
	<div class="fnl_div columns_ fifth_width different_color" id="FNL" data-etat="4">
		<div class="todo_titre"><b>'.$langs->trans('Facturées_non_livrées').'</b><span class="filter_in_etat" id="nbr_FNL"/></span></div>
		<div class="contents">
...
print '</div><div class="hover_bkgr_fricc">
    ...
    <div class="windows_pop nc_pop">
        ...
        <div class="window-header">
	        ...
	        <div class="window-title" id="commande_title">
		        <h2 class="title"></h2>
	        </div>
	        <div>
	    		Client : <span id="commande_tiers"></span>
	        </div>
	        ...
	        <div>
	        	...
	        	<p style="margin-bottom: 0;"><textarea id="commande_comment" class="textarea_comment" rows="2" onkeyup="comment_change()" placeholder="'.$langs->trans("Écrivez_un_commentaire").'…"></textarea></p>
...
<?php
dol_include_once('/teamview/commandes/js/commandes_functions.php');
