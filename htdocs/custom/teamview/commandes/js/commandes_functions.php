...
function load_all_commandes(){
	...
	var data = {
		...
		'offset' : offset,
		'filter' : filter,
		'action' : "getallcommandes"
	};
	$.ajax({
		type: "POST",
		url: "<?php echo dol_buildpath('/teamview/commandes/check.php',2); ?>",
		data: data, 
		dataType: 'json',
...
function OpenCommandePop($t){
	var id_commande = $($t).find('.id_commande').val();
	var data = {
	...
	$.ajax({
		...
		success: function(found){
			if (found) {
				// console.log("Commande found : "+found);
				$("#commande_title").html(found["ref"]);
				$("#commande_tiers").html(found["tiers"]);
	...
	$(".one_content").draggable({
  	...
  	$(".scroll_div").droppable({
		...
		drop: function (event, ui) {
			...
			var id_commande = ui.draggable.data("rowid");

			var arr = [];
			if (to == "DRAFT")
				arr = [];
			else if (to == "VALIDATED")
				arr = ["DRAFT","LNF","LF","FNL"];
			else if (to == "LNF")
				arr = ["VALIDATED","LF","FNL"];
			else if (to == "LF")
				arr = ["VALIDATED","LNF","FNL"];
			else if (to == "FNL")
				arr = ["VALIDATED","LNF","LF"];
			...
			if (to !== from && permis == 1) {
				...
				if (arr.indexOf(from) >= 0){
					if(from == "DRAFT" && ui.draggable.data("lines") == 0){
						msg = "<?php echo trim(addslashes($langs->trans("commandeVide"))); ?>";
...
function get_files_joints() {
	var id_commande 	= $("#commande_title").find('.title').attr('id');
...
function to_delete_file($t, e, cmntId){
    ...
    if ($('.one_comment#comment_'+cmntId).find('.commande_comment_edit').val() == "")
...
function progress_tasks_change(){
	var all100 = true;
	$('#tablelines > tbody  > tr .commande_avanc').each(function() {
		tot = $(this).find('select option:selected').val();
		if (tot < 100)
			all100 = false;
	});
	if (all100){
		$("#commande_avance select").val(100);
		$("#commande_avance select").trigger('change');
		// createorupdatetask();
	}
...
function update_comment($t) {
	...
	var id_comment 	= $($t).parent().parent().find(".id_comment").val();
	var comment 	= $($t).parent().find(".commande_comment_edit").val();
...
function create_comment($t) {
	var id_commande 	= $("#commande_title").find('.title').attr('id');
	var comment 	= $("#commande_comment").val();
